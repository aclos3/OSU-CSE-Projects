/***************************************************************************
**   Program Name: Final Project (Agate.cpp)
**   Author: Andrew Clos
**   Date: 8/12/2019
...
Agate::Agate()
{
	roomName = "\033[1;93mAgate\033[0m";
	numDigSites = 28;
	costPerDig = 1;
	odds = 95;
	ptsPerGem = 10;
...
********************************************************************************/
std::vector<int> Agate::playerEnters(int shovels)
{
	...
	while (menuChoice != 3)
	{
		...
		if (menuChoice == 2)
		{
			...
			{  //the option to interact with the object in the room is presented.
				std::cout << "You also notice a mysterious, glittering pile of old mining ore in the corner." << std::endl;
				std::cout << "(1) Attempt to sift through the Ore -- this could be risky." << std::endl;
				...
				{   //if user chooses to interact, there is an equal chance that something good, neutral or bad will happpen.
					...
					std::cout << "You scratch at the ore with your pick..." << std::endl;
					//good fortune
					if (randomObject == 1)
					{
						encounter = getRandom(5, 10);
					...
					else if (randomObject == 3)
					{
						...
						std::cout << "Oh no, a " << roomName << " rat jumps up and steals " << encounter << " of your shovels, that scoundrel!" << std::endl;
			...
			{   //if the object has been interacted with on an earlier call to look around, it cannot be interacted with again.
				std::cout << "...And the sifted-through ore." << std::endl;
			}
		}
...
void Agate::lookAround()
{
	...
	std::cout << "\033[1;93m  (&%%%%%%%#%%% *,,**,.,,*//*///(((*(/****//(**,**.(%#(%#%%&%#%%%%%&%#%%#%%%%%&&%&%&&&&%%%%(       \033[0m" << std::endl;
	std::cout << "\033[1;93m   #%%%%%%%%%%%%%%#((* .,,. *(//(#(* ,((((/,..((#%%#%#%%&&%&&&%%&&&%%%##&%&&&&&&&&&%%%%%#        \033[0m" << std::endl;
	std::cout << "\033[1;93m    #%%%%&&&&&&&&&&%%%%%%&%%%&%&&%&%#%###%%%%%#%&%#(#%&&%%%#&&&%&&&&&%&%%%%&&&&%%%%%%%%%%,  \033[0m" << std::endl << std::endl;
