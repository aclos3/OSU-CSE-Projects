/***************************************************************************
**   Program Name: Final Project (Aquamarine.cpp)
**   Author: Andrew Clos
**   Date: 8/12/2019
...
{
	roomName = "\033[1;96mAquamarine\033[0m";
	numDigSites = 8;
	costPerDig = 4;
	odds = 99;
	ptsPerGem = 25;
...
********************************************************************************/
std::vector<int> Aquamarine::playerEnters(int shovels)
{
	...
	while (menuChoice != 3)
	{
		...
		if (menuChoice == 2)
		{
			...
			{  //the option to interact with the object in the room is presented.
				std::cout << "You also notice a small chest under some rags." << std::endl;
				std::cout << "(1) Attempt to Open the Chest -- this could be risky." << std::endl;
				...
				{   //if user chooses to interact, there is an equal chance that something good, neutral or bad will happpen.
					...
					std::cout << "You pry at the dusty lock mechanism..." << std::endl;
					//good fortune
					if (randomObject == 1)
					{
						encounter = getRandom(3, 7);
			...
			{   //if the object has been interacted with on an earlier call to look around, it cannot be interacted with again.
				std::cout << "...And the splintered open chest." << std::endl;
			}
		}
