/***************************************************************************
**   Program Name: Final Project (Emerald.cpp)
**   Author: Andrew Clos
**   Date: 8/12/2019
...
Emerald::Emerald()
{
	roomName = "\033[1;92mEmerald\033[0m";
	numDigSites = 16;
	costPerDig = 2;
	odds = 45;
	ptsPerGem = 115;
...
********************************************************************************/
std::vector<int> Emerald::playerEnters(int shovels)
{
	...
	while (menuChoice != 3)
	{
		...
		if (menuChoice == 2)
		{
			...
			{  //the option to interact with the object in the room is presented.
				std::cout << "You also notice a glittering vase on a small wooden table near the door to the room." << std::endl;
				std::cout << "(1) Investigate the vase -- this could be risky." << std::endl;
				...
				{   //if user chooses to interact, there is an equal chance that something good, neutral or bad will happpen.
					...
					std::cout << "The top appears to be sealed, ah heck -- just smash it on the ground..." << std::endl;
					//good fortune
					if (randomObject == 1)
					{
						encounter = getRandom(1, 4);
					...
					{
						std::cout << "Nothing but some old flower stems here." << std::endl;
					}
					//bad fortune
			...
			{   //if the object has been interacted with on an earlier call to look around, it cannot be interacted with again.
				std::cout << "...And the obliterated vase." << std::endl;
			}
		}
...
****************************************************************************/
void Emerald::lookAround()
{
	//print ASCII art unique to each room. 
	//emerald
	std::cout << "\033[1;92m          .*///((((((#((((((((((((############(####(####################%#%%%%%####%(             \033[0m" << std::endl;
	std::cout << "\033[1;92m         ./(%#%%%%%&&%&&%%&%%%&%%%&&&&&&&&&&&&%%%&%&%&&&&&&&&&&&&&&&&&&&&&&&&&&&%&&%(%%#           \033[0m" << std::endl;
	std::cout << "\033[1;92m       .((##(/###%%%%%%%%%%%%%%%&&&&&&&&&&&&&&&&&&%&%&&&&&&&&&&&&&&&&&%&&&&&&%&&&&%#%&&&%#         \033[0m" << std::endl;
	std::cout << "\033[1;92m      (((######%##%%%%%%%%%%%%%%%&&&&&&%&&&%%&%%&%&%%&&&&&%&%&&&&&&&%%%%%%%&%%&&&%%&%%%%&&%(       \033[0m" << std::endl;
	std::cout << "\033[1;92m    (((#%%%#%%%%###%%%%#####%%%%%%%&&&%%%%%%%%%%%%%%%%%%%%&%&&%&&%&&%&%%%%%%#####%%%%%%%%%#%%/     \033[0m" << std::endl;
