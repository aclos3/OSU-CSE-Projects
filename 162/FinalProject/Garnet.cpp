/***************************************************************************
**   Program Name: Final Project (Garnet.cpp)
**   Author: Andrew Clos
**   Date: 8/12/2019
...
Garnet::Garnet()
{
	roomName = "\033[3;100;30mGarnet\033[0m";
	numDigSites = 20;
	costPerDig = 2;
	odds = 85;
	ptsPerGem = 10;
...
********************************************************************************/
std::vector<int> Garnet::playerEnters(int shovels)
{
	...
	while (menuChoice != 3)
	{
		...
		if (menuChoice == 2)
		{
			...
			{  //the option to interact with the object in the room is presented.
				std::cout << "You also notice a small leather pouch sitting on a cabinent." << std::endl;
				std::cout << "(1) Open the pouch -- this could be risky." << std::endl;
				...
				{   //if user chooses to interact, there is an equal chance that something good, neutral or bad will happpen.
					...
					std::cout << "You peel back the crusty, dried leather..." << std::endl;
					//good fortune
					if (randomObject == 1)
					{
						encounter = getRandom(4, 11);
					...
					{
						std::cout << "Nothing but some old rags and blasting caps are present here." << std::endl;
					}
					//bad fortune
					else if (randomObject == 3)
					{
						...
						std::cout << "Oh no, a " << roomName << " mouse scurries away with " << encounter << " of your shovels, that scoundrel!" << std::endl;
			...
			{   //if the object has been interacted with on an earlier call to look around, it cannot be interacted with again.
				std::cout << "...And the husk of the leather pouch." << std::endl;
			}
		}
...
****************************************************************************/
void Garnet::lookAround()
{
	//print ASCII art unique to each room. 
	//garnet
	std::cout << "\033[3;100;30m                                    ..                                                             \033[0m" << std::endl;
	std::cout << "\033[3;100;30m                                 .%&&%%%#((                                                        \033[0m" << std::endl;
	std::cout << "\033[3;100;30m                                .#&&%%&&&%&&%#                                                     \033[0m" << std::endl;
	std::cout << "\033[3;100;30m                               .(&&&&&&%(&&&&&%(#,                                                 \033[0m" << std::endl;
	std::cout << "\033[3;100;30m                              ,,&&@&&&&&&@@@&&&&&&%((                                              \033[0m" << std::endl;
	...
	std::cout << "\033[3;100;30m                   (%%%%%%######%%%%%%&&@@@&&&&&&%((#((/. .,((#%%%%&&&&&%%#((//*                   \033[0m" << std::endl;
	std::cout << "\033[3;100;30m                             ..((#######%%&@@&&%&&%&&&%%%&&&&&%#%%#*.                              \033[0m" << std::endl;
	std::cout << "\033[3;100;30m                                                  ///////***,***                                   \033[0m" << std::endl << std::endl;
